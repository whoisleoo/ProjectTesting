...
export async function gerarImagem(curso, periodo, turma) {
    ...
    try {
        ...
        const bernardo = `${prefixo}${periodo}${turma}`;
        const moaca = `${prefixo}${periodo}`;
        let index = -1;
        ...
        if (pages.findIndex(i => i.includes(bernardo)) !== -1) {
            index = pages.findIndex(i => i.includes(bernardo));
        } else if (pages.findIndex(i => i.includes(moaca)) !== -1) {
            index = pages.findIndex(i => i.includes(moaca));
        }
        ...
        if (pageLine.findIndex(i => i.includes(bernardo)) !== -1) {
            barbosaLine = pageLine.findIndex(i => i.includes(bernardo));
        } else if (pageLine.findIndex(i => i.includes(moaca)) !== -1) {
            barbosaLine = pageLine.findIndex(i => i.includes(moaca));
        }

        const ratio = barbosaLine / pageLine.length;
        ...
        resultado = await convert(index + 1);
...
router.get('/ensalamento', async (req, res) => {
    ...
    try {
        ...
        let imgBuffer;
        try {
            imgBuffer = await gerarImagem(curso, periodo, turma);
        } catch (err) {
            if (err.message === 'PDF not found') return res.status(404).json({ error: "PDF was not found." });
            if (err.message === 'Turma not found') return res.status(404).json({ error: "Turma with this name was not found. " });
            throw err;
        }
        ...
        if (canal === 'discord') {
            if (!webhook) return res.status(404).json({ error: "Webhook não informado. " });

            fs.writeFileSync(tempImg, imgBuffer);
            const embed = new MessageBuilder()
                .setTitle('Seu horário de aula!')
                .setDescription(`────────────────────── ⋆⋅☆⋅⋆ ────────────────────── \n\n ⭐ Ensalamento de **${curso}** do periodo **${periodo}** da turma **${turma}** \n 🧟 Developed by whoisleoo \n\n ────────────────────── ⋆⋅☆⋅⋆ ──────────────────────`)
                .setImage('attachment://ensalamento.png')
                .setColor('#ffff00');
